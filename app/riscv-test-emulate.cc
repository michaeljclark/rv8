...
struct riscv_emulator
{
	...
	*/
	...
	{
		void *addr = mmap((void*)(stack_top - stack_size), stack_size, PROT_READ | PROT_WRITE, MAP_FIXED | MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);
		if (addr == MAP_FAILED) {
	    ...
	    */
		...
		printf("guest stack =  0x%016" PRIxPTR " - 0x%016" PRIxPTR "\n",
			(stack_top - stack_size), stack_top);

		return addr;
	}

	void* map_load_segment(const char* filename, void *vaddr, size_t len, size_t offset)
	{
		...
		/*
			TODO - map PT_LOAD segments using ELF permissions
			Simple code currently maps all segments copy-on-write RWX
	    */

		void *addr = mmap(vaddr, len, PROT_READ | PROT_WRITE | PROT_EXEC, MAP_FIXED | MAP_PRIVATE, fd, offset);
		if (addr == MAP_FAILED) {
			panic("map_executable: error: mmap: %s: %s", filename, strerror(errno));
		}
		close(fd);

		printf("guest text  =  0x%016" PRIxPTR " - 0x%016" PRIxPTR "\n",
			uintptr_t(vaddr), uintptr_t(vaddr) + len);
	...
	void run()
	{
		...
		for (size_t i = 0; i < elf.phdrs.size(); i++) {
			...
			if (phdr.p_flags & PT_LOAD) {
				...
				map_load_segment(filename.c_str(), (void*)phdr.p_vaddr, phdr.p_memsz, phdr.p_offset);
			}
		}
