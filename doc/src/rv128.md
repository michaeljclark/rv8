...
- The layout would be equivalent to AMO, so funct5 encoding space and width is encoding available for the opcode
...
- Use either one or two major opcodes if there is a need to un-alias register slots in 31:27 and 11:7 i.e. move function selection to increase encoding space
- Increased encoding space would allow additional metadata bit for LX and SX to differentiate between code and data pointers
