...
std::vector<cmdline_option> riscv_gen_cc::get_cmdline_options()
{
...
static std::string opcode_substitute(riscv_opcode_ptr opcode)
{
...
static void print_fpu_asm_cc(riscv_gen *gen)
{
...
static int rm = 0;
	...
	printf(kCHeader, "test-fpu-asm.cc");
	printf("%s", kFpuSourceHeader);

	std::vector<std::pair<riscv_extension_ptr,std::string>> test_cases;
		...
		switch (operand_list.size()) {
			case 2: {
				...
				for (auto &v1 : values1) {
					...
					printf("\tprintf(\"d=%s\\n\", d);\n", operand_fmt[0].c_str());
					printf("}\n\n");
					test_cases.push_back(std::pair<riscv_extension_ptr,std::string>(ext, test_name));
				}
				break;
			}
			case 3: {
