...
namespace riscv {
	...
	struct time_mmio_device : memory_segment<typename P::ux>
	{
		...
		void store_32(UX va, u32 val)
		{
			...
			if (va == 0) mtime = (mtime & (u64(-1) << 32)) | val;
			else if (va == 4) mtime = (mtime & (u64(-1) >> 32)) | (u64(val) << 32);
			else if (va == 8) mtimecmp = (mtimecmp & (u64(-1) << 32)) | val;
			else if (va == 12) mtimecmp = (mtimecmp & (u64(-1) >> 32)) | (u64(val) << 32);
		}
