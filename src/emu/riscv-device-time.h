...
namespace riscv {
	...
	struct time_mmio_device : memory_segment<typename P::ux>
	{
		...
		void load_8 (UX va, u8  &val) { if (proc.log & proc_log_mmio) printf("time_mmio:0x%04llx -> invalid\n", addr_t(va)); val = 0; }
		void load_16(UX va, u16 &val) { if (proc.log & proc_log_mmio) printf("time_mmio:0x%04llx -> invalid\n", addr_t(va)); val = 0; }
		...
		void load_32(UX va, u32 &val)
		{
			...
			if (proc.log & proc_log_mmio) printf("time_mmio:0x%04llx -> 0x%04x\n", addr_t(va), val);
		}
		...
		void load_64(UX va, u64 &val)
		{
			...
			if (proc.log & proc_log_mmio) printf("time_mmio:0x%04llx -> 0x%08llx\n", addr_t(va), val);
		}

		void store_8 (UX va, u8  val) { if (proc.log & proc_log_mmio) printf("time_mmio:0x%04llx <- invalid\n", addr_t(va)); }
		void store_16(UX va, u16 val) { if (proc.log & proc_log_mmio) printf("time_mmio:0x%04llx <- invalid\n", addr_t(va)); }
