...
inline uint64_t riscv_decode_inst_op(uint64_t inst)
{
...
template <typename T>
inline void riscv_decode_inst_type(T &dec, uint64_t inst)
{
	dec.codec = riscv_inst_codec[dec.op];
	switch (dec.codec) {
		case riscv_codec_none:          riscv::decode_none(dec, inst);                     break;
		case riscv_codec_u:             riscv::decode_u(dec, inst);                        break;
		case riscv_codec_uj:            riscv::decode_uj(dec, inst);                       break;
		case riscv_codec_i:             riscv::decode_i(dec, inst);                        break;
		case riscv_codec_i_sh5:         riscv::decode_i_sh5(dec, inst);                    break;
		case riscv_codec_i_sh6:         riscv::decode_i_sh6(dec, inst);                    break;
		case riscv_codec_s:             riscv::decode_s(dec, inst);                        break;
		case riscv_codec_sb:            riscv::decode_sb(dec, inst);                       break;
		case riscv_codec_r:             riscv::decode_r(dec, inst);                        break;
		case riscv_codec_r_m:           riscv::decode_r_m(dec, inst);                      break;
		...
		case riscv_codec_cb_sh6:        riscv::decode_cb_sh6(dec, inst);                   break;
		case riscv_codec_ci:            riscv::decode_ci(dec, inst);                       break;
		case riscv_codec_ci_sh5:        riscv::decode_ci_sh5(dec, inst);                   break;
		case riscv_codec_ci_sh6:        riscv::decode_ci_sh6(dec, inst);                   break;
		case riscv_codec_ci_16sp:       riscv::decode_ci_16sp(dec, inst);                  break;
		case riscv_codec_ci_lwsp:       riscv::decode_ci_lwsp(dec, inst);                  break;
		case riscv_codec_ci_ldsp:       riscv::decode_ci_ldsp(dec, inst);                  break;
		case riscv_codec_ci_li:         riscv::decode_ci_li(dec, inst);                    break;
		case riscv_codec_ci_lui:        riscv::decode_ci_lui(dec, inst);                   break;
		...
		case riscv_codec_cr_mv:         riscv::decode_cr_mv(dec, inst);                    break;
		case riscv_codec_cr_jalr:       riscv::decode_cr_jalr(dec, inst);                  break;
		case riscv_codec_cr_jr:         riscv::decode_cr_jr(dec, inst);                    break;
		case riscv_codec_cs:            riscv::decode_cs(dec, inst);                       break;
		case riscv_codec_cs_sw:         riscv::decode_cs_sw(dec, inst);                    break;
		case riscv_codec_cs_sd:         riscv::decode_cs_sd(dec, inst);                    break;
		case riscv_codec_css_swsp:      riscv::decode_css_swsp(dec, inst);                 break;
		case riscv_codec_css_sdsp:      riscv::decode_css_sdsp(dec, inst);                 break;
	};
}
...
template <typename T>
inline uint64_t riscv_encode_inst(T &dec)
{
	dec.codec = riscv_inst_codec[dec.op];
	...
	switch (dec.codec) {
		...
		case riscv_codec_uj:            return inst |= riscv::encode_uj(dec);              break;
		case riscv_codec_i:             return inst |= riscv::encode_i(dec);               break;
		case riscv_codec_i_sh5:         return inst |= riscv::encode_i_sh5(dec);           break;
		case riscv_codec_i_sh6:         return inst |= riscv::encode_i_sh6(dec);           break;
		case riscv_codec_s:             return inst |= riscv::encode_s(dec);               break;
		case riscv_codec_sb:            return inst |= riscv::encode_sb(dec);              break;
		case riscv_codec_r:             return inst |= riscv::encode_r(dec);               break;
		case riscv_codec_r_m:           return inst |= riscv::encode_r_m(dec);             break;
		...
		case riscv_codec_cb_sh6:        return inst |= riscv::encode_cb_sh6(dec);          break;
		case riscv_codec_ci:            return inst |= riscv::encode_ci(dec);              break;
		case riscv_codec_ci_sh5:        return inst |= riscv::encode_ci_sh5(dec);          break;
		case riscv_codec_ci_sh6:        return inst |= riscv::encode_ci_sh6(dec);          break;
		case riscv_codec_ci_16sp:       return inst |= riscv::encode_ci_16sp(dec);         break;
		case riscv_codec_ci_lwsp:       return inst |= riscv::encode_ci_lwsp(dec);         break;
		case riscv_codec_ci_ldsp:       return inst |= riscv::encode_ci_ldsp(dec);         break;
		case riscv_codec_ci_li:         return inst |= riscv::encode_ci_li(dec);           break;
		case riscv_codec_ci_lui:        return inst |= riscv::encode_ci_lui(dec);          break;
		...
		case riscv_codec_cr_mv:         return inst |= riscv::encode_cr_mv(dec);           break;
		case riscv_codec_cr_jalr:       return inst |= riscv::encode_cr_jalr(dec);         break;
		case riscv_codec_cr_jr:         return inst |= riscv::encode_cr_jr(dec);           break;
		case riscv_codec_cs:            return inst |= riscv::encode_cs(dec);              break;
		case riscv_codec_cs_sw:         return inst |= riscv::encode_cs_sw(dec);           break;
		case riscv_codec_cs_sd:         return inst |= riscv::encode_cs_sd(dec);           break;
		case riscv_codec_css_swsp:      return inst |= riscv::encode_css_swsp(dec);        break;
		case riscv_codec_css_sdsp:      return inst |= riscv::encode_css_sdsp(dec);        break;
	};
	return inst;
}
