	...
	struct fusion_emitter : fusion_base<P>
	{
		...
		void emit_sign_extend_32(decode_type &dec)
		{
			...
			if (rdx > 0) {
				...
				rv::as.movsxd(x86::rdx, rv::rbp_reg_d(dec.rd));
				rv::as.mov(rv::rbp_reg_q(dec.rd), x86::rdx);
				log_trace("\t\tmovsxd rdx, %s", rv::rbp_reg_str_d(dec.rd));
		...
		bool emit_mul(decode_type &dec)
		{
			...
			if (dec.rd == rv_ireg_zero) {
				...
				// imul rd, rs2
				if (rs2x > 0) {
					rv::as.imul(x86::gpq(rdx), x86::gpq(rs2x));
					log_trace("\t\timul %s, %s", rv::x86_reg_str_q(rdx), rv::x86_reg_str_q(rs2x));
				} else {
					rv::as.imul(x86::gpq(rdx), rv::rbp_reg_q(dec.rs2));
					log_trace("\t\timul %s, %s", rv::x86_reg_str_q(rdx), rv::rbp_reg_str_q(dec.rs2));
				}
			}
			else if (dec.rd == dec.rs2 && rdx > 0) {
				// imul rd, rs1
				if (rs1x > 0) {
					rv::as.imul(x86::gpq(rdx), x86::gpq(rs1x));
					log_trace("\t\timul %s, %s", rv::x86_reg_str_q(rdx), rv::x86_reg_str_q(rs1x));
				} else {
					rv::as.imul(x86::gpq(rdx), rv::rbp_reg_q(dec.rs1));
					log_trace("\t\timul %s, %s", rv::x86_reg_str_q(rdx), rv::rbp_reg_str_q(dec.rs1));
				}
			}
			else if (rdx < 0) {
				// mov rax, rs1
				...
				if (rs2x > 0) {
					rv::as.imul(x86::rax, x86::gpq(rs2x));
					log_trace("\t\timul rax, %s", rv::x86_reg_str_q(rs2x));
				} else {
					rv::as.imul(x86::rax, rv::rbp_reg_q(dec.rs2));
					log_trace("\t\timul rax, %s", rv::rbp_reg_str_q(dec.rs2));
				}
				rv::as.mov(rv::rbp_reg_q(dec.rd), x86::rax);
				log_trace("\t\tmov %s, rax", rv::rbp_reg_str_q(dec.rd));
			}
		...
		bool emit_mulh(decode_type &dec)
		{
			...
			if (dec.rd == rv_ireg_zero) {
				...
				if (rs2x > 0) {
					rv::as.imul(x86::gpq(rs2x));
					log_trace("\t\timul %s", rv::x86_reg_str_q(rs2x));
				} else {
					rv::as.mov(x86::rdx, rv::rbp_reg_q(dec.rs2));
					rv::as.imul(x86::rdx);
					log_trace("\t\tmov rdx, %s", rv::rbp_reg_str_q(dec.rs2));
					log_trace("\t\timul rdx");
				}
		...
		bool emit_mulhu(decode_type &dec)
		{
			...
			if (dec.rd == rv_ireg_zero) {
				...
				if (rs2x > 0) {
					...
					log_trace("\t\tmul %s", rv::x86_reg_str_q(rs2x));
				} else {
					rv::as.mov(x86::rdx, rv::rbp_reg_q(dec.rs2));
					rv::as.mul(x86::rdx);
					log_trace("\t\tmov rdx, %s", rv::rbp_reg_str_q(dec.rs2));
		...
		bool emit_mulhsu(decode_type &dec)
		{
			...
			if (dec.rd == rv_ireg_zero) {
				...
				/* multiply, negate and stash result in rdx */
				if (rs2x > 0) {
					rv::as.mul(x86::gpq(rs2x));
					log_trace("\t\tmul %s", rv::x86_reg_str_q(rs2x));
				} else {
				...
				rv::as.xor_(x86::rdx, Imm(-1));
				rv::as.mov(x86::rdx, x86::rdx);
				log_trace("\t\txor rdx, -1");
				...
				rv::as.lea(x86::rdx, x86::qword_ptr(x86::rdx, x86::rax));
				rv::as.jmp(out);
				log_trace("\t\ttest rax, rax");
				log_trace("\t\tsetz al");
				log_trace("\t\tmovzx eax, al");
				log_trace("\t\tlea rdx, qword ptr [rdx + rax]");
				log_trace("\t\tjmp out");
				...
				if (rs2x > 0) {
					...
					rv::as.mov(x86::rdx, rv::rbp_reg_q(dec.rs2));
					rv::as.mul(x86::rdx);
					log_trace("\t\tmov", rv::rbp_reg_str_q(dec.rs2));
					log_trace("\t\tmul rdx");
				}
		...
		bool emit_div(decode_type &dec)
		{
			...
			if (dec.rd == rv_ireg_zero) {
				...
				// mov rd, -1
				if (rdx > 0) {
					rv::as.mov(x86::gpq(rdx), Imm(-1));
					log_trace("\t\tmov %s, -1", rv::x86_reg_str_q(rdx), dec.imm);
				} else {
					rv::as.mov(rv::rbp_reg_q(dec.rd), Imm(-1));
					log_trace("\t\tmov %s, -1", rv::rbp_reg_str_q(dec.rd), dec.imm);
				}
			}
			else if (dec.rs1 == rv_ireg_zero) {
				emit_zero_rd(dec);
				...
				Label div2 = rv::as.newLabel();

				rv::as.mov(x86::rax, std::numeric_limits<int64_t>::min());
				log_trace("\t\tmov rax, %lld", std::numeric_limits<int64_t>::min());
				if (rs1x > 0) {
					rv::as.cmp(x86::gpq(rs1x), x86::rax);
					log_trace("\t\tcmp %s, rax", rv::x86_reg_str_q(rs1x));
				} else {
					rv::as.cmp(rv::rbp_reg_q(dec.rs1), x86::rax);
					log_trace("\t\tcmp %s, rax", rv::rbp_reg_str_q(dec.rs1));
				}
				rv::as.jne(div1);
				log_trace("\t\tjne div1");
				...
				if (rs2x > 0) {
					...
					rv::as.cmp(rv::rbp_reg_q(dec.rs2), Imm(-1));
					log_trace("\t\tcmp %s, -1", rv::rbp_reg_str_q(dec.rs2));
				}
				rv::as.je(out);
				log_trace("\t\tje out");
				...
				if (rs2x > 0) {
					...
					log_trace("\t\tidiv %s", rv::x86_reg_str_q(rs2x));
				} else {
				...
				if (rdx > 0) {
					...
					log_trace("\t\tmov %s, rax", rv::x86_reg_str_q(rdx));
				} else {
					rv::as.mov(rv::rbp_reg_q(dec.rd), x86::rax);
					log_trace("\t\tmov %s, rax", rv::rbp_reg_str_q(dec.rd));
				}
		...
		bool emit_divu(decode_type &dec)
		{
			...
			if (dec.rd == rv_ireg_zero) {
				...
				if (rs2x > 0) {
					...
					log_trace("\t\tdiv %s", rv::x86_reg_str_q(rs2x));
				} else {
		...
		bool emit_addw(decode_type &dec)
		{
			...
			if (dec.rd == rv_ireg_zero) {
				...
				if (rdx > 0) {
					if (rs2x > 0) {
						rv::as.movsxd(x86::gpq(rdx), x86::gpd(rs2x));
						log_trace("\t\tmovsxd %s, %s", rv::x86_reg_str_q(rdx), rv::x86_reg_str_d(rs2x));
					} else {
						rv::as.mov(x86::gpq(rdx), rv::rbp_reg_d(dec.rs2));
						log_trace("\t\tmovsxd %s, %s", rv::x86_reg_str_q(rdx), rv::rbp_reg_str_d(dec.rs2));
					}
				} else {
					if (rs2x > 0) {
						rv::as.movsxd(x86::rax, x86::gpd(rs2x));
						rv::as.mov(rv::rbp_reg_q(dec.rd), x86::rax);
						log_trace("\t\tmovsxd rax, %s", rv::x86_reg_str_d(rs2x));
						log_trace("\t\tmov %s, rax", rv::rbp_reg_str_q(dec.rd));
					} else {
						rv::as.movsxd(x86::rax, rv::rbp_reg_d(dec.rs2));
						rv::as.mov(rv::rbp_reg_q(dec.rd), x86::rax);
						log_trace("\t\tmovsxd rax, %s", rv::rbp_reg_str_d(dec.rs2));
						log_trace("\t\tmov %s, rax", rv::rbp_reg_str_q(dec.rd));
					}
				}
			}
				...
				if (rdx > 0) {
					if (rs1x > 0) {
						...
						rv::as.mov(x86::gpq(rdx), rv::rbp_reg_d(dec.rs1));
						log_trace("\t\tmovsxd %s, %s", rv::x86_reg_str_q(rdx), rv::rbp_reg_str_d(dec.rs1));
					}
				} else {
					if (rs1x > 0) {
				...
				if (rdx > 0) {
					if (rs2x > 0) {
						rv::as.add(x86::gpd(rdx), x86::gpd(rs2x));
						log_trace("\t\tadd %s, %s", rv::x86_reg_str_d(rdx), rv::x86_reg_str_d(rs2x));
					} else {
						rv::as.add(x86::gpd(rdx), rv::rbp_reg_d(dec.rs2));
						log_trace("\t\tadd %s, %s", rv::x86_reg_str_d(rdx), rv::rbp_reg_str_d(dec.rs2));
					}
				} else {
					if (rs2x > 0) {
						rv::as.add(rv::rbp_reg_d(dec.rd), x86::gpd(rs2x));
						log_trace("\t\tadd %s, %s", rv::rbp_reg_str_d(dec.rd), rv::x86_reg_str_d(rs2x));
					} else {
						rv::as.mov(x86::eax, rv::rbp_reg_d(dec.rs2));
						rv::as.add(rv::rbp_reg_d(dec.rd), x86::eax);
						log_trace("\t\tmov eax, %s", rv::rbp_reg_str_d(dec.rs2));
						log_trace("\t\tadd %s, eax", rv::rbp_reg_str_d(dec.rd));
					}
				}
				...
				if (rdx > 0) {
					if (rs1x > 0) {
						rv::as.add(x86::gpd(rdx), x86::gpd(rs1x));
						log_trace("\t\tadd %s, %s", rv::x86_reg_str_d(rdx), rv::x86_reg_str_d(rs1x));
					} else {
						rv::as.add(x86::gpd(rdx), rv::rbp_reg_d(dec.rs1));
						log_trace("\t\tadd %s, %s", rv::x86_reg_str_d(rdx), rv::rbp_reg_str_d(dec.rs1));
					}
				} else {
					if (rs1x > 0) {
						rv::as.add(rv::rbp_reg_d(dec.rd), x86::gpd(rs1x));
						log_trace("\t\tadd %s, %s", rv::rbp_reg_str_d(dec.rd), rv::x86_reg_str_d(rs1x));
					} else {
						rv::as.mov(x86::eax, rv::rbp_reg_d(dec.rs1));
						rv::as.add(rv::rbp_reg_d(dec.rd), x86::eax);
						log_trace("\t\tmov eax, %s", rv::rbp_reg_str_d(dec.rs1));
				...
				rv::as.lea(x86::gpd(rdx), x86::dword_ptr(x86::gpd(rs1x), x86::gpd(rs2x)));
				log_trace("\t\tlea %s, dword ptr [%s + %s]",
					rv::x86_reg_str_d(rdx), rv::x86_reg_str_d(rs1x), rv::x86_reg_str_d(rs2x));
				emit_sign_extend_32(dec);
			}
			else if (rdx < 0 && (rs1x < 0 || rs2x < 0)) {
				// mov eax, rs1
				...
				if (rs1x > 0) {
					...
					log_trace("\t\tmov eax, %s", rv::x86_reg_str_d(rs1x));
				} else {
					rv::as.mov(x86::eax, rv::rbp_reg_d(dec.rs1));
					log_trace("\t\tmov eax, %s", rv::rbp_reg_str_d(dec.rs1));
				}
				if (rs2x > 0) {
					rv::as.add(x86::eax, x86::gpd(rs2x));
					log_trace("\t\tadd eax, %s", rv::x86_reg_str_d(rs2x));
				} else {
					rv::as.add(x86::eax, rv::rbp_reg_d(dec.rs2));
					log_trace("\t\tadd eax, %s", rv::rbp_reg_str_d(dec.rs2));
				}
				rv::as.movsxd(x86::rax, x86::eax);
				rv::as.mov(rv::rbp_reg_q(dec.rd), x86::rax);
		...
		bool emit_subw(decode_type &dec)
		{
			...
			if (dec.rd == rv_ireg_zero) {
				...
				// neg rd
				if (rdx > 0) {
					rv::as.neg(x86::gpd(rdx));
					log_trace("\t\tneg %s", rv::x86_reg_str_d(rdx));
				} else {
					rv::as.neg(rv::rbp_reg_d(dec.rd));
					log_trace("\t\tneg %s", rv::rbp_reg_str_d(dec.rd));
				}
				emit_sign_extend_32(dec);
			}
			else if (dec.rs2 == rv_ireg_zero) {
				...
				if (rdx > 0) {
					if (rs2x > 0) {
						rv::as.sub(x86::gpd(rdx), x86::gpd(rs2x));
						log_trace("\t\tsub %s, %s", rv::x86_reg_str_d(rdx), rv::x86_reg_str_d(rs2x));
					} else {
						rv::as.sub(x86::gpd(rdx), rv::rbp_reg_d(dec.rs2));
						log_trace("\t\tsub %s, %s", rv::x86_reg_str_d(rdx), rv::rbp_reg_str_d(dec.rs2));
					}
				} else {
					if (rs2x > 0) {
						rv::as.sub(rv::rbp_reg_d(dec.rd), x86::gpd(rs2x));
						log_trace("\t\tsub %s, %s", rv::rbp_reg_str_d(dec.rd), rv::x86_reg_str_d(rs2x));
					} else {
						rv::as.mov(x86::eax, rv::rbp_reg_d(dec.rs2));
						rv::as.sub(rv::rbp_reg_d(dec.rd), x86::eax);
						log_trace("\t\tmov eax, %s", rv::rbp_reg_str_d(dec.rs2));
						log_trace("\t\tsub %s, eax", rv::rbp_reg_str_d(dec.rd));
					}
				}
				emit_sign_extend_32(dec);
			}
				...
				if (rdx > 0) {
					if (rs1x > 0) {
						rv::as.sub(x86::gpd(rdx), x86::gpd(rs1x));
						log_trace("\t\tsub %s, %s", rv::x86_reg_str_d(rdx), rv::x86_reg_str_d(rs1x));
					} else {
						rv::as.sub(x86::gpd(rdx), rv::rbp_reg_d(dec.rs1));
						log_trace("\t\tsub %s, %s", rv::x86_reg_str_d(rdx), rv::rbp_reg_str_d(dec.rs1));
					}
				} else {
					if (rs1x > 0) {
						rv::as.sub(rv::rbp_reg_d(dec.rd), x86::gpd(rs1x));
						log_trace("\t\tsub %s, %s", rv::rbp_reg_str_d(dec.rd), rv::x86_reg_str_d(rs1x));
					} else {
						rv::as.mov(x86::eax, rv::rbp_reg_d(dec.rs1));
						rv::as.sub(rv::rbp_reg_d(dec.rd), x86::eax);
						log_trace("\t\tmov eax, %s", rv::rbp_reg_str_d(dec.rs1));
				...
				if (rs2x > 0) {
					rv::as.sub(x86::eax, x86::gpd(rs2x));
					log_trace("\t\tsub eax, %s", rv::x86_reg_str_d(rs2x));
				} else {
					rv::as.sub(x86::eax, rv::rbp_reg_d(dec.rs2));
					log_trace("\t\tsub eax, %s", rv::rbp_reg_str_d(dec.rs2));
				}
				rv::as.movsxd(x86::rax, x86::eax);
				rv::as.mov(rv::rbp_reg_q(dec.rd), x86::rax);
				log_trace("\t\tmovsxd rax, eax");
		...
		bool emit_mulw(decode_type &dec)
		{
			...
			if (dec.rd == rv_ireg_zero) {
				...
				// imul rd, rs2
				if (rs2x > 0) {
					rv::as.imul(x86::gpd(rdx), x86::gpd(rs2x));
					log_trace("\t\timul %s, %s", rv::x86_reg_str_d(rdx), rv::x86_reg_str_d(rs2x));
				} else {
					rv::as.imul(x86::gpd(rdx), rv::rbp_reg_d(dec.rs2));
					log_trace("\t\timul %s, %s", rv::x86_reg_str_d(rdx), rv::rbp_reg_str_d(dec.rs2));
				}
				emit_sign_extend_32(dec);
			}
			else if (dec.rd == dec.rs2 && rdx > 0) {
				// imul rd, rs1
				if (rs1x > 0) {
					rv::as.imul(x86::gpd(rdx), x86::gpd(rs1x));
					log_trace("\t\timul %s, %s", rv::x86_reg_str_d(rdx), rv::x86_reg_str_d(rs1x));
				} else {
					rv::as.imul(x86::gpd(rdx), rv::rbp_reg_d(dec.rs1));
					log_trace("\t\timul %s, %s", rv::x86_reg_str_d(rdx), rv::rbp_reg_str_d(dec.rs1));
				}
				emit_sign_extend_32(dec);
			}
			else if (rdx < 0) {
				...
				if (rs2x > 0) {
					rv::as.imul(x86::eax, x86::gpd(rs2x));
					log_trace("\t\timul eax, %s", rv::x86_reg_str_d(rs2x));
				} else {
					rv::as.imul(x86::eax, rv::rbp_reg_d(dec.rs2));
					log_trace("\t\timul eax, %s", rv::rbp_reg_str_d(dec.rs2));
				}
				rv::as.mov(rv::rbp_reg_d(dec.rd), x86::eax);
				log_trace("\t\tmov %s, eax", rv::rbp_reg_str_d(dec.rd));
				emit_sign_extend_32(dec);
		...
		bool emit_divw(decode_type &dec)
		{
			...
			if (dec.rd == rv_ireg_zero) {
				...
				rv::as.mov(x86::eax, std::numeric_limits<int32_t>::min());
				log_trace("\t\tmov eax, %d", std::numeric_limits<int32_t>::min());
				if (rs1x > 0) {
					rv::as.cmp(x86::gpd(rs1x), x86::eax);
					log_trace("\t\tcmp %s, eax", rv::x86_reg_str_d(rs1x));
				} else {
					rv::as.cmp(rv::rbp_reg_d(dec.rs1), x86::eax);
					log_trace("\t\tcmp %s, eax", rv::rbp_reg_str_d(dec.rs1));
				}
				rv::as.jne(div1);
				log_trace("\t\tjne div1");
				...
				if (rs2x > 0) {
					...
					rv::as.cmp(rv::rbp_reg_d(dec.rs2), Imm(-1));
					log_trace("\t\tcmp %s, -1", rv::rbp_reg_str_d(dec.rs2));
				}
				rv::as.je(out);
				log_trace("\t\tje out");
				...
				if (rs2x > 0) {
					...
					log_trace("\t\tidiv %s", rv::x86_reg_str_d(rs2x));
				} else {
				...
				if (rdx > 0) {
					...
					log_trace("\t\tmovsxd %s, eax", rv::x86_reg_str_q(rdx));
				} else {
					rv::as.movsxd(x86::rax, x86::eax);
					rv::as.mov(rv::rbp_reg_q(dec.rd), x86::rax);
					log_trace("\t\tmovsxd rax, eax");
		...
		bool emit_remw(decode_type &dec)
		{
			...
			if (dec.rd == rv_ireg_zero) {
				...
				if (rdx > 0) {
					...
					log_trace("\t\tmovsxd %s, edx", rv::x86_reg_str_q(rdx));
				} else {
					rv::as.movsxd(x86::rdx, x86::edx);
					rv::as.mov(rv::rbp_reg_q(dec.rd), x86::rdx);
		...
		bool emit_divuw(decode_type &dec)
		{
			...
			if (dec.rd == rv_ireg_zero) {
				...
				if (rs2x > 0) {
					...
					log_trace("\t\tdiv %s", rv::x86_reg_str_d(rs2x));
				} else {
		...
		bool emit_sd(decode_type &dec)
		{
			...
			if (dec.rs2 == rv_ireg_zero) {
				...
				if (rs2x > 0) {
					...
					log_trace("\t\tmov rdx, %s", rv::rbp_reg_str_q(dec.rs2));
					if (rs1x > 0) {
						rv::as.mov(x86::qword_ptr(x86::gpq(rs1x), dec.imm), x86::rdx);
						log_trace("\t\tmov qword ptr [%s + %lld], rdx", rv::x86_reg_str_q(rs1x), dec.imm);
					} else {
						rv::as.mov(x86::rax, rv::rbp_reg_q(dec.rs1));
						rv::as.mov(x86::qword_ptr(x86::rax, dec.imm), x86::rdx);
						log_trace("\t\tmov rax, %s", rv::rbp_reg_str_q(dec.rs1));
		...
		bool emit_sw(decode_type &dec)
		{
			...
			if (dec.rs2 == rv_ireg_zero) {
				...
				if (rs2x > 0) {
					...
					log_trace("\t\tmov rdx, %s", rv::rbp_reg_str_q(dec.rs2));
					if (rs1x > 0) {
						rv::as.mov(x86::dword_ptr(x86::gpq(rs1x), dec.imm), x86::edx);
						log_trace("\t\tmov dword ptr [%s + %lld], edx", rv::x86_reg_str_q(rs1x), dec.imm);
					} else {
						rv::as.mov(x86::rax, rv::rbp_reg_q(dec.rs1));
						rv::as.mov(x86::dword_ptr(x86::rax, dec.imm), x86::edx);
						log_trace("\t\tmov rax, %s", rv::rbp_reg_str_q(dec.rs1));
						log_trace("\t\tmov dword ptr [rax + %lld], edx", dec.imm);
					}
				}
			}
			return true;
		...
		bool emit_sh(decode_type &dec)
		{
			...
			if (dec.rs2 == rv_ireg_zero) {
				...
				if (rs2x > 0) {
					...
					log_trace("\t\tmov rdx, %s", rv::rbp_reg_str_q(dec.rs2));
					if (rs1x > 0) {
						rv::as.mov(x86::word_ptr(x86::gpq(rs1x), dec.imm), x86::dx);
						log_trace("\t\tmov word ptr [%s + %lld], dx", rv::x86_reg_str_q(rs1x), dec.imm);
					} else {
						rv::as.mov(x86::rax, rv::rbp_reg_q(dec.rs1));
						rv::as.mov(x86::word_ptr(x86::rax, dec.imm), x86::dx);
						log_trace("\t\tmov rax, %s", rv::rbp_reg_str_q(dec.rs1));
						log_trace("\t\tmov word ptr [rax + %lld], dx", dec.imm);
					}
				}
			}
			return true;
		...
		bool emit_sb(decode_type &dec)
		{
			...
			if (dec.rs2 == rv_ireg_zero) {
				...
				if (rs2x > 0) {
					...
					log_trace("\t\tmov rdx, %s", rv::rbp_reg_str_q(dec.rs2));
					if (rs1x > 0) {
						rv::as.mov(x86::byte_ptr(x86::gpq(rs1x), dec.imm), x86::dl);
						log_trace("\t\tmov byte ptr [%s + %lld], dl", rv::x86_reg_str_q(rs1x), dec.imm);
					} else {
						rv::as.mov(x86::rax, rv::rbp_reg_q(dec.rs1));
						rv::as.mov(x86::byte_ptr(x86::rax, dec.imm), x86::dl);
						log_trace("\t\tmov rax, %s", rv::rbp_reg_str_q(dec.rs1));
						log_trace("\t\tmov byte ptr [rax + %lld], dl", dec.imm);
					}
				}
			}
			return true;
