...
struct S<K,L,H,T...> : S<K,L,T...>
{
	...
	static inline constexpr uint64_t decode(uint64_t insn) {
		...
		return ((shift < 0 ? insn << -shift : insn >> shift) & mask) |
			I::decode(insn);
	}
...
struct imm_impl_t<R,W,H,T...> : imm_impl_t<R,W,T...>
{
	...
	static inline constexpr R decode(uint64_t insn) {
		return I::decode(insn) | H::decode(insn);
	}
...
struct simm_t : imm_impl_t<int64_t,W,Args...>
{
	...
	static constexpr int64_t decode(uint64_t insn) {
		return sign_extend<int64_t,W>(I::decode(insn));
	}
...
struct uimm_t : imm_impl_t<uint64_t,W,Args...>
{
	...
	static constexpr uint64_t decode(uint64_t insn) {
		return I::decode(insn);
	}
